...
namespace ARSoft.Tools.Net.Dns
{
    ...
    public class DnsKeyRecord : DnsRecordBase
	{
		...
		{
			X9ECParameters x9Params = ECGost3410NamedCurves.GetByOid(CryptoProObjectIdentifiers.GostR3410x2001CryptoProA);
			ECDomainParameters dParams = new ECDomainParameters(x9Params);
		...
		public static DnsKeyRecord CreateSigningKey(DomainName name, RecordClass recordClass, int timeToLive, DnsKeyFlags flags, byte protocol, DnsSecAlgorithm algorithm, int keyStrength = 0)
		{
			...
			switch (algorithm)
			{
				...
				case DnsSecAlgorithm.EccGost:
					X9ECParameters x9Params = ECGost3410NamedCurves.GetByOid(CryptoProObjectIdentifiers.GostR3410x2001CryptoProA);
					ECDomainParameters gostEcDomainParameters = new ECDomainParameters(x9Params);
